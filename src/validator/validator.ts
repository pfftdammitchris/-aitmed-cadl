...
export default (async function () {
  $(document).ready(function () {
    $('#cadlEndpoint-btn').click(function () {
      ...
      $.when(fetchAll(url))
        .then(function (res) {
          ...
          const { valid, invalid } = res.reduce(
            (acc: Acc, obj: CADLResponse) => {
          ...
          //add details
          $('.cadlEndpoint-details').html(
            `<span style='font-weight:500'>Total #</span>: ${
              valid.length + invalid.length
            }  <span style='color:green;font-weight:500'>VALID</span>: ${
              valid.length
            }  <span style='color:red;font-weight:500'>INVALID</span>: ${
              invalid.length
            }`
          )
          ...
          $.each(invalid, function (i) {
            ...
            $.each(errors, function (j) {
              const a = $('<a/>').addClass('error').text(errors[j]).appendTo(li)
              $('<br/>').appendTo(a)
            })
          })
        })
        .catch((err) => {
          $('#val-endpoint-text').hide()
          $('.cadlEndpoint-details').html(
            "<span style='color:red;font-weight:400'>INVALID</span>"
          )
          $('.cadlEndpoint-error').html(
            `<span style='color:red;font-weight:400'>${err}</span>`
          )
        })
    })
    ...
    $('#cadlPage-btn').click(function () {
      ...
      $.when(fetchCADLObject(url))
        .then(function () {
          $('#val-page-text').hide()
          $('#isValid').html(
            "<span style='color:green;font-weight:400'>VALID</span>"
          )
        })
        .catch((err) => {
          $('#val-page-text').hide()
