...
function get({ pageName, apiObject, dispatch }) {
    return async () => {
        ...
        try {
            ...
            res = data
            if (store.env === 'test') {
                console.log('%cGet Document Response', 'background: purple; color: white; display: block;',
                    res
                );
            }
        } catch (error) {
        ...
        if (res) {
            dispatch({
                ...
                payload: {
                    pageName, dataKey: dataOut ? dataOut : dataKey,
                    data: res
                }
            })
            dispatch({
                ...
                payload: {
                    pageName,
                    newVal: res, dataKey: dataOut ? dataOut : dataKey
                }
            })
        }
        return res
...
function create({ pageName, apiObject, dispatch }) {
    return async ({ data, type }) => {
        ...
        if (id) {
            try {
                if (store.env === 'test') {
                    console.log('%cUpdate Document Request', 'background: purple; color: white; display: block;',
                        { ...options, id }
                    );
                }
                ...
                res = data
                if (store.env === 'test') {
                    console.log('%cUpdate Document Response', 'background: purple; color: white; display: block;',
                        res
                    );
                }
            } catch (error) {
            ...
            try {
                ...
                res = response
                if (store.env === 'test') {
                    console.log('%cCreate Document Response', 'background: purple; color: white; display: block;',
                        res
                    );
                }
            } catch (error) {
